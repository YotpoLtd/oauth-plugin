...
class OAuthEcho
  def call(env)
    ...
    response[:account] = env["oauth.client_application"].key if env["oauth.client_application"]
  ...
  describe 'OAuth1' do
    describe 'with optional white space' do
      ...
      it "should sign with oauth 1 access token" do
        client_application = Account.new "my_consumer"
        Account.stub!(:find_by_key).and_return(client_application)
